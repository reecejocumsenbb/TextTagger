...
# Exact match: https://www.linkedin.com/search/results/all/?keywords="autistic"
# Exact match: https://www.linkedin.com/search/results/all/?keywords=autistic
...
class QuoraScraper():
    ...
    def scrape(self):
        ...
        driver = webdriver.Firefox(options=options, executable_path='./scraper/geckodriver.exe')
        ...
        for keyword in self.keywords:
            ...
            if not self.streamlit: 
                pbar = tqdm(ncols=80, total=self.posts_per_keyword, unit='post',)
            else:
            ...
            while True:
                ...
                posts = [post for post in soup.findAll('span', {'class': 'qt_truncated_inline'}) if post.get_text() != '' and not post.get_text().isspace()]
                ...
                #self.display(f'Got {len(posts)} posts this iteration')
                if not self.streamlit: 
                    pbar.update((min(len(sentences), self.posts_per_keyword)) - pbar.n)
                else:
                    pbars[keyword].progress((min(len(sentences), self.posts_per_keyword)/self.posts_per_keyword))
            ...
            self.display(f'Got {len(sentences)} for "{keyword}", using {self.posts_per_keyword}')
            data[keyword] = list(sentences)[:self.posts_per_keyword]
            overall_count += self.posts_per_keyword
        self.display(f'Total of {overall_count} sentences gathered in {time.time() - start} seconds.')
        if self.streamlit:
            json_string = json.dumps(data)
            self.output_container.download_button(label="As .json", data=json_string, file_name='sentences.json', mime='application/json')
        else:
